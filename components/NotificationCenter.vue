...
const { state, refreshUnreadCount, markRead, markAllRead } = useNotifications()
const api = useTaskFlowApi()
const dropdownState = reactive<{ notifications: TaskFlowNotification[]; loading: boolean; error: string | null }>({
...
const loadDropdown = async () => {
  ...
  dropdownState.error = null
  try {
    const response = await api.getNotifications({ page: 1, page_size: 10 })
    dropdownState.notifications = response.results
  } catch (error) {
    dropdownState.error = taskFlowApiErrorMessage(error, 'Could not load notifications')
    throw error
  } finally {
...
const toggle = async () => {
  ...
  if (open.value) {
    ...
    await nextTick()
    updatePanelPosition()
    try { await loadDropdown() } catch { /* error is rendered */ }
  }
}
...
const select = async (notification: TaskFlowNotification) => {
  try {
    ...
    dropdownState.notifications = dropdownState.notifications.map(item => item.id === updated.id ? updated : item)
  } catch { return }
  open.value = false
...
const allRead = async () => {
  try {
    await markAllRead({ reload: false })
    dropdownState.notifications = dropdownState.notifications.map(item => ({ ...item, is_read: true, read_at: item.read_at || new Date().toISOString() }))
  } catch { /* rolled back */ }
}
const retry = async () => { try { await loadDropdown() } catch { /* rendered */ } }
const viewAll = () => {
  open.value = false
  emit('viewAll')
}
  ...
  <div ref="root" class="relative z-[60]">
    ...
    <div v-if="open" ref="panel" :style="panelStyle" :class="['tf-notification-dropdown fixed z-[9999] flex flex-col overflow-hidden rounded-[18px] border shadow-2xl', props.dark ? 'is-dark-mode' : '']">
      ...
      <div class="min-h-0 flex-1 overflow-y-auto">
        <div v-if="dropdownState.loading && !dropdownState.notifications.length" class="space-y-3 p-4"><div v-for="i in 3" :key="i" class="h-20 animate-pulse rounded-xl bg-slate-100" /></div>
        <div v-else-if="dropdownState.error" class="p-8 text-center"><p class="text-sm text-task-danger">{{ dropdownState.error }}</p><button type="button" class="mt-3 text-sm font-bold text-task-blue" @click="retry">Try again</button></div>
        <p v-else-if="!dropdownState.notifications.length" class="px-5 py-12 text-center text-sm text-task-muted">No notifications yet</p>
        <div v-else class="divide-y divide-task-line"><NotificationItem v-for="item in dropdownState.notifications" :key="item.id" :notification="item" @select="select" /></div>
      </div>
