...
const fields: Array<{ key: keyof NotificationPreferences; label: string; hint: string }> = [
  { key: 'task_assigned', label: 'Task assignments', hint: 'Get notified when a new task is assigned to you.' },
  { key: 'deadline_reminder', label: 'Deadline reminders', hint: 'Get a reminder when a task deadline is approaching.' },
  { key: 'task_overdue', label: 'Overdue tasks', hint: 'Get notified when a task becomes overdue.' },
  { key: 'task_updates', label: 'Task updates', hint: 'Get notified when a task status changes.' },
  { key: 'team_messages', label: 'Team messages', hint: 'Get notified about new chat messages.' }
]

const load = async () => {
  loading.value = true; error.value = ''
  try { Object.assign(preferences, await api.getNotificationPreferences()) }
  catch (e) { error.value = taskFlowApiErrorMessage(e, 'Could not load preferences') }
...
const update = async (key: keyof NotificationPreferences) => {
  ...
  catch (e) { preferences[key] = previous; error.value = taskFlowApiErrorMessage(e, 'Could not save preference') }
...
<template>
  <section class="tf-panel p-5 sm:p-7">
    <div class="flex items-start gap-3"><span class="grid h-10 w-10 place-items-center rounded-xl bg-task-blueSoft text-task-blue"><svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9Zm-8.3 12a2.5 2.5 0 0 0 4.6 0" /></svg></span><div><h2 class="text-xl font-bold">Notification preferences</h2><p class="mt-1 text-sm text-task-muted">Choose which events you want to be notified about.</p></div></div>
    ...
    <div v-if="error" class="mt-4 flex items-center justify-between rounded-xl bg-task-dangerSoft px-4 py-3 text-xs text-task-danger"><span>{{ error }}</span><button type="button" class="font-bold" @click="load">Try again</button></div>
  </section>
</template>
