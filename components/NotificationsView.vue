...
<template>
  <section class="tf-panel mx-auto w-full max-w-5xl overflow-hidden p-0">
    ...
    <footer v-if="!state.loading && pageCount > 1" class="border-t border-task-line px-4 py-3"><AppPagination :page="page" :page-count="pageCount" :total="state.totalCount" :page-size="pageSize" noun="notifications" compact @page="page = $event" /></footer>
  </section>
</template>
